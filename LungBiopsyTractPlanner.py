           ...
           # --- 5) Aday traktları topla ------------------------------
            ...
            for batch_start in range(0, len(skin_pts), batch_size):
                ...
                for ras, nrm in zip(batch_pts, batch_nrms):
                    ...
                    candidates.append((p0, p_end, lung_mm, px, hmr, px_t, hmr_h, d))
            ...
            if not candidates:
                raise RuntimeError("Hiç aday trakt bulunamadı!")

            
            
            # --- 6) Her noktadan en risksiz trakt ---------------------
            unique = {}
            for ras, p_end, lung_mm, px, hmr, px_t, hmr_h, d in candidates:
                key = tuple(np.round(ras,0))
                if key not in unique or px<unique[key][3]:
                    unique[key] = (ras, p_end, lung_mm, px, hmr, px_t, hmr_h, d)
            filtered = list(unique.values())
            
            
            

            # --- 7) Px_sort ve Hedefe yakınlığa göre sırala -----------------------
            filtered.sort(key=lambda x: (x[2], x[3], x[4], x[7]))
            
            

            # --- 8) Greedy + seyreltme ----------------------------
            selected = []
            for ras, p_end, lung_mm, px, hmr, px_t, hmr_h, d in filtered:
                if not selected:
                    selected.append((ras, p_end, lung_mm, px, hmr, px_t, hmr_h, d)); continue
                dists = [np.linalg.norm(ras - sras) for sras,_,_,_,_,_,_,_ in selected]
                if min(dists)>=MIN_DIST_BETWEEN_POINTS:
                    selected.append((ras, p_end, lung_mm, px, hmr, px_t, hmr_h, d))

            print(f"[INFO] Greedy + seyreltme sonrası trakt sayısı: {len(selected)}")
      
            selected_tracts = {
                "pneumothorax": sorted(selected, key=lambda x: (x[2], x[7], x[3], x[4]))[:10]
            }

            # --- Seçilen traktları tekilleştir ve listele ---
            all_sel = selected_tracts["pneumothorax"]
            unique_sel = {}
            for ras, p_end, lung_mm, px, hmr, px_t, hmr_h, d in all_sel:
                key = tuple(np.round(ras,0))
                if key not in unique_sel:
                    unique_sel[key] = (ras, p_end, lung_mm, px, hmr, px_t, hmr_h, d)
            final_selected = list(unique_sel.values())

            # --- 10) Zorluk skorunu hesapla, sırala ve çiz ----
            tracts = []
            for ras, p_end, lung_mm, px, hmr, px_t, hmr_h, d in final_selected:
